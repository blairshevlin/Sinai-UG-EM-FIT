...
for i = 1:nP
    ...
    while good < 8
        ...
        [offer_ic_0, choice_ic_0] = simulate_0step_ic_noNorm(gen_params(i,1:4));
        if sum(choice_ic_0) ~= 30 && sum(choice_ic_0) ~= 0
            good = good +1;
        end
%         good = good +1;
        [offer_nc_0, choice_nc_0] = simulate_0step_nc_noNorm(gen_params(i,1:4));
        if sum(choice_nc_0) ~= 30 && sum(choice_nc_0) ~= 0
            good = good +1;
        end
        

        [offer_ic_1, choice_ic_1] = simulate_1step_ic_noNorm(gen_params(i,1:5));
        if sum(choice_ic_1) ~= 30 && sum(choice_ic_1) ~= 0
           good = good +1;
        end
        

        [offer_nc_1, choice_nc_1] = simulate_1step_nc_noNorm(gen_params(i,[1:4,6]));
        if sum(choice_nc_1) ~= 30 && sum(choice_nc_1) ~= 0
            good = good +1;
        end

        [offer_ic_2, choice_ic_2] = simulate_2step_ic_noNorm(gen_params(i,1:5));
        if sum(choice_ic_2) ~= 30 && sum(choice_ic_2) ~= 0
            good = good +1;
        end

        [offer_nc_2, choice_nc_2] = simulate_2step_nc_noNorm(gen_params(i,[1:4,6]));
        if sum(choice_nc_2) ~= 30 && sum(choice_nc_2) ~= 0
            good = good +1;
        end

        [offer_ic_3, choice_ic_3] = simulate_3step_ic_noNorm(gen_params(i,1:5));
        if sum(choice_ic_3) ~= 30 && sum(choice_ic_3) ~= 0
            good = good +1;
        end

        [offer_nc_3, choice_nc_3] = simulate_3step_nc_noNorm(gen_params(i,[1:4,6]));
...
%%
save('Hackathon-Data_Combined-IC-NC_s150_noFlat_noNorm.mat', 's');
