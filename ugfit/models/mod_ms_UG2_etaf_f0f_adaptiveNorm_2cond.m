function [fval,fit] = mod_ms_UG2_etaf_f0f_adaptiveNorm_2cond(behavData,q, doprior,dofit,varargin)
...
[qt, bounds] = norm2par('ms_UG2_etaf_f0f_adaptiveNorm_2cond',q); % transform parameters from gaussian space to model space
...
delta_IC = qt(4); % expected influence
if (delta_IC<min(bounds(:,4)) || delta_IC>max(bounds(:,4))), fval=10000000; return; end

delta_NC = qt(5); % expected influence
if (delta_NC<min(bounds(:,5)) || delta_NC>max(bounds(:,5))), fval=10000000; return; end


fixed = [0.8, 10];
free = {alpha beta epsilon delta_IC delta_NC};
% -------------------------------------------------------------------------------------
...
%end
if doprior == 1
    [fval,~,~,ChoiceProb] = lik_UG2_etaf_f0f_adaptiveNorm_2cond(behavData.offer, behavData.choice,fixed,free,doprior,prior,q);
else
    [fval,~,~,ChoiceProb] = lik_UG2_etaf_f0f_adaptiveNorm_2cond(behavData.offer, behavData.choice,fixed,free,doprior);
...
if dofit ==1
   fit         = struct;
   fit.q = qt;
   fit.xnames  = {'alpha'; 'beta';'epsilon';'delta_IC';'delta_NC'};
