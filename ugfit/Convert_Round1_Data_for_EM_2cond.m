...
% 2cond refers to the fact that both conditions are combined during fitting
% Created by BRK Shevlin, Dec 2023
clear;
...
for i = 1:size(possIDs,2)
    ...
    keepStruct.offer{1} = IC_offer(:,i);
    keepStruct.offer{2} = NC_offer(:,i);

    keepStruct.choice{1} = IC_Choice(:,i);
    keepStruct.choice{2} = NC_Choice(:,i);
...
save('DATA_COVID_Round1_2cond_noFlat-BOTH_passAttn_allData_Dec15.mat','s');
