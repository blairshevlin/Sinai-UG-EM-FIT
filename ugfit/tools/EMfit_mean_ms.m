...
if quickfit==1, fit.convCrit= 0.1; else fit.convCrit= 1e-4; end
fit.maxit   = 1000; 
fit.maxEvals= 500;
...
for iiter = 1:fit.maxit                         
   ...
   fprintf(['.']);
   %fprintf(['\n , ',num2str(mean(NPL(:,iiter)))]);

   % Is there a way to use the mean value to assess convergence
   if abs(mean(NPL(:,iiter))-NPL_old) < fit.convCrit  && flagcov==1            % if  MAP estimate does not improve and covariances were properly computed
    
       fprintf('...converged!!!!! \n');  nextbreak=1;                          % stops at end of this loop iteration                                                        
   end
   NPL_old = mean(NPL(:,iiter));
   
   
   % ============== Plot progress (for visualisation only ===================
   %NLL(iiter) = mean(NPL(:,iiter)) - mean(NLPrior(:,iiter));                    % compute NLL manually, just for visualisation
 %  subplot(2,1,1);
 %  plot(mean(NPL(:,1:iiter)),'b'); hold all;
 %  plot(NLL(1:iiter),'r'); 
...
for is = 1:n_subj
   ...
   if abs(modout.(modelID).fit.nll(is)-nll_check) >.0000000001
       disp(is); 
   end
       %disp('ERROR'); keyboard; end % check nll calculation
end
