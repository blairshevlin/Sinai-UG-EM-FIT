function [offer, resp] = simulate_1step_nc_unboundFS(free)
    ...
    for i = 1:n       
        ...
        CV(i) =FS_unbound(envy, offer(i), norm(i+1));
        FVa(i) = max(0, FS_unbound(envy,  max(offer(i)-delta, mn), norm(i+1)));
        FVr(i) = max(0, FS_unbound(envy,  max(offer(i)+delta, mn), norm(i+1)));
