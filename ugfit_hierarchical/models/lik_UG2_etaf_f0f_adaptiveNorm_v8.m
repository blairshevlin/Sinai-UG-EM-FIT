function [fval, norm, V, ChoiceProb] = lik_UG2_etaf_f0f_adaptiveNorm_v8(offer, resp, fixed, free, doprior, varargin)
   %v2 prioritizes early information (trials < 20)
    ...
    early = 1:(n/2);
    late = (n/2)+1:n;
    log_probs(early) = log( ChoiceProb(early) ) * 4;
    log_probs(late) = log( ChoiceProb(late) ) * 4;
